...
"99",c.id,a.$config.accessToken,a.$config.appId].join("/"),function(a){(d||b.noop)(a)})},refresh:function(b){d.valid.str(b)?a.$config.scope.bodyUrl=b+(0<=b.indexOf("?")?"&":"?")+d.date.timestamp():d.valid.obj(a.$config.scope.$subnav)?a.$config.scope.bodyUrl=a.$config.scope.$subnav.path+(0<=a.$config.scope.$subnav.path.indexOf("?")?"&":"?")+d.date.timestamp():console.error("wrong url or nav setting")},fullscreen:function(b){a.$config.scope.$fullscreen=d.valid.defined(b)?b:!0!==a.$config.scope.$fullscreen;
